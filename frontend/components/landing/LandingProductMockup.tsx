  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 border-b border-white/10 px-4 py-3 sm:px-6 lg:px-8">
        ...
        <p className="ml-2 truncate text-left text-xs text-emerald-200 md:text-sm">
      ...
      <div className="flex min-h-[220px] flex-col md:min-h-0 md:flex-row">
            ...
            {NAV_KEYS.map((key) => {
              ...
              return (
                <li
                  ...
                  className={
                    active
                      ? "rounded-md border border-emerald-500/35 bg-emerald-500/15 px-2 py-1.5 font-medium text-emerald-200"
                      : "rounded-md px-2 py-1.5 text-emerald-300"
                  }
                >
                  {tNav(key)}
                </li>
        ...
        <div className="flex flex-1 flex-col gap-3 p-4 text-left sm:gap-4 md:p-6">
          <div>
            <h3 className="text-sm font-semibold text-emerald-100">{tDash("snapshotTitle")}</h3>
            <dl className="mt-3 grid grid-cols-2 gap-3 sm:grid-cols-4 sm:gap-4">
              <div>
                <dt className="text-xs text-emerald-300">{tDash("income")}</dt>
                <dd className="font-figures mt-0.5 text-sm font-medium tabular-nums text-emerald-50 md:text-base">
                  {fmt.format(DEMO.income)}
                </dd>
              </div>
              <div>
                <dt className="text-xs text-emerald-300">{tDash("expenses")}</dt>
                <dd className="font-figures mt-0.5 text-sm font-medium tabular-nums text-emerald-50 md:text-base">
                  {fmt.format(DEMO.expenses)}
                </dd>
              </div>
              <div>
                <dt className="text-xs text-emerald-300">{tDash("surplusLabel")}</dt>
                <dd className="font-figures mt-0.5 text-sm font-medium tabular-nums text-emerald-200 md:text-base">
                  {fmt.format(DEMO.surplus)}
                </dd>
              </div>
              <div>
                <dt className="text-xs text-emerald-300">{tDash("savings")}</dt>
                <dd className="font-figures mt-0.5 text-sm font-medium tabular-nums text-emerald-50 md:text-base">
          ...
          <p className="text-sm leading-relaxed text-emerald-100">
            {tDash("quickForecast")}{" "}
            <span className="font-figures font-semibold text-emerald-200">
          ...
          <div>
            <div className="flex justify-between text-xs text-emerald-300">
              ...
              <span className="font-figures text-emerald-100">{DEMO.foundationPct}%</span>
            </div>
            ...
            <p className="mt-2 text-xs leading-snug text-emerald-300">{tDash("foundationHint")}</p>
          </div>
        </div>
      </div>
    </div>
